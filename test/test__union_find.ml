...
let%test_unit "union" =
  let a = create 1 in
  let b = create 2 in
  assert (same_class a a);
  assert (same_class b b);
...
let%test_unit "balanced" =
  let log2 n =
    Int.of_float (Float.round_up (Float.log (Float.of_int n) /. Float.log 2.))
  in
  let n = 1000 in
...
(* This test was disabled due to the following exception:

   {v
    (Failure "Gc.Memprof.start: not implemented in multicore")
   v}

   {[
     let%expect_test "[get] of a root does not allocate" =
       let t = create () in
       Expect_test_helpers_core.require_no_allocation [%here] (fun () -> get t)
     ;;
   ]} *)
